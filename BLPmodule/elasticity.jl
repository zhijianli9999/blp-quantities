...
function compute_elasticity(
    ...
    positionin::Matrix{Int64}, #positionin[jj,tt] is jj's position in tt
    n_firms::Int
)::Vector{Float64}
    ...
    Threads.@threads for jj in 1:n_firms
        η[jj] = reduce(+,[
                        mktq[tt][positionin[jj,tt]] *
                        (1-share[tt][positionin[jj,tt]]) * 
                        θ * 
                        X[tt][positionin[jj,tt]]
                    for tt in t_indexer[jj]
                ]) / 
                reduce(+,[mktq[tt][positionin[jj,tt]] for tt in t_indexer[jj]])
    end
    return η
end
